...
try {
  ...
  await page.evaluate(() => {
    ...
    window.records = [
      { id: "one", seq: 1, kind: "user", text: "Native canonical transcript" },
    ];
  });
  await page.getByText("Working", { exact: true }).waitFor();
  await page.getByText("Transcript", { exact: true }).click();
  await page
    .getByText("Native canonical transcript", { exact: true })
    .waitFor();
  assert.equal(
    await page.evaluate(
      () =>
  ...
  await page.evaluate(() => (window.voiceError = "Voice connection failed"));
  await page
    .getByRole("alert")
    .getByText("Voice connection failed", { exact: true })
    .waitFor();
  ...
  await page
    .getByRole("alert")
    .filter({ hasText: "The microphone is off." })
    .waitFor();
  assert.equal(await page.evaluate(() => window.stops), 1);
  await reset();
  await start();
  ...
  await page
    .getByRole("alert")
    .filter({ hasText: "Voice could not reconnect" })
