...
class SpawnTask(unittest.TestCase):
    ...
    def test_failed_or_deleted_worker_releases_unfinished_work(self):
        ...
        with self.rt.lock, self.rt.db() as db:
            ...
            released = self.rt.release_failed_work(db, self.rt.records(db, 'agents'), force=True)
            again = self.rt.release_failed_work(db, self.rt.records(db, 'agents'), force=True)
            throttled = self.rt.release_failed_work(db, self.rt.records(db, 'agents'))
        self.assertEqual((released, again, throttled), ([self.task['id']], [], []))
        task = self.work()
